...
def get_similar_listings(
    ...
    try:
        ...
        if not similar_listings.empty:
            ...
            similar_listings["price_diff_percent"] = similar_listings[
                "price_diff_percent"
            ].round(1)
            ...
            if "post_date" in similar_listings.columns:
                ...
                similar_listings["post_date_display"] = similar_listings[
                    "post_date"
                ].dt.strftime("%d/%m/%Y")

            # Tạo URL đầy đủ nếu chưa có
            if (
                "url" in similar_listings.columns
                and "url_full" not in similar_listings.columns
            ):
                similar_listings["url_full"] = similar_listings["url"].apply(
                    lambda url: (
                        f"https://xe.chotot.com{url}"
...
def get_models(brand):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT DISTINCT model_normalized FROM motorbikes WHERE brand = ? order by model_normalized",
            (brand,),
        )
...
def load_market_data():
    ...
    try:
        ...
        return pd.DataFrame(
            columns=["Thương hiệu", "Số lượng giao dịch", "Giá trung bình (triệu VND)"]
        )
...
def analyze_image(img_str):
    ...
    try:
        ...
        response = requests.post(
            CLAUDE_API_URL, headers=headers, json=payload, timeout=30
        )
        ...
        try:
            ...
            # Chuẩn hóa keys
            normalized_result = {
                "brand": parsed_result.get(
                    "brand", parsed_result.get("Thương hiệu", "Unknown")
                ),
                "model": parsed_result.get(
                    "model", parsed_result.get("Mẫu xe", "Unknown")
                ),
                "year": parsed_result.get(
                    "year", parsed_result.get("Năm sản xuất", 2020)
                ),
                "cc": parsed_result.get(
                    "cc", parsed_result.get("Dung tích động cơ", "Unknown")
                ),
                "condition": parsed_result.get(
                    "condition", parsed_result.get("Tình trạng xe", "Tốt")
                ),
    ...
    try:
        ...
        try:
            ...
            normalized_result = {
                ...
                "km_driven": parsed_result.get(
                    "km_driven", parsed_result.get("Số km đã đi", 15000)
                ),
                "condition": parsed_result.get(
                    "condition", parsed_result.get("Tình trạng xe", "Tốt")
                ),
