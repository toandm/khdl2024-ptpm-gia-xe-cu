...
def train_model(data_path, output_path):
    ...
    try:
        ...
        logger.info(
            f"Đã xử lý dữ liệu thành công. Ma trận đặc trưng có kích thước: {X.shape}"
        )
    except Exception as e:
    ...
    logger.info(
        f"Kích thước tập huấn luyện: {X_train.shape}, Kích thước tập kiểm tra: {X_test.shape}"
    )
    ...
    try:
        ...
        rf_model = RandomForestRegressor(
            ...
            max_depth=10,
            min_samples_split=2,
            # Bỏ đi tham số `max_features`. Hiện tại đang để giá trị mặc định "auto".
            # Từ phiên bản 1.1 trở đi, giá trị "auto" được thay thế bởi "sqrt". Nếu để
            # "auto" sẽ gây lỗi.
            # Ref: https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestClassifier.html#:~:text=Changed%20in%20version%201.1%3A%20The%20default%20of%20max_features%20changed%20from%20%22auto%22%20to%20%22sqrt%22.
            min_samples_leaf=1,
            random_state=42,
            n_jobs=-1,
        )
    ...
    try:
        ...
        metrics_path = os.path.join(
            os.path.dirname(output_path), "rf_model_metrics.json"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Huấn luyện mô hình dự đoán giá xe máy"
    )
    parser.add_argument(
        "--data",
        default="data/raw/chotot_data.csv",
        help="Đường dẫn đến dữ liệu huấn luyện",
    )
    parser.add_argument(
        "--output", default="models/", help="Đường dẫn để lưu mô hình đã huấn luyện"
    )
